...
class Message(object):
    ...
    def build_shift_dict(self, shift):
        ...
        for alpha in (string.ascii_lowercase, string.ascii_uppercase):
            alphabet += zip(alpha, alpha[shift:] + alpha[0:shift])
...
class CiphertextMessage(Message):
    ...
    def decrypt_message(self):
        ...
        best_msg = ""
        for i in range(26):
            dec_text = self.apply_shift(i)
            words = dec_text.split()
            real_word = sum([is_word(self.get_valid_words(),w) for w in words])
            if real_word > best_real_words:
                best_shift = i
                best_real_words = real_word
                best_msg = dec_text
...
Solution:
def decrypt_story():
    storytime = CiphertextMessage(get_story_string())
    return storytime.decrypt_message()
