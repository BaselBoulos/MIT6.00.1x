...
while balance !=0 and balance > 0:
    ...
    while months > 0:
        months -= 1
        monthlyunpaid = (balance) - (minfixed)
        monthlyinterestR = (annualInterestRate) / 12.0
        balance = (monthlyunpaid) + (monthlyinterestR * monthlyunpaid)
    months = 12
print(minfixed)
'''
...
You can try running your code locally so that the payment can be any dollar and cent amount
(in other words, the monthly payment is a multiple of $0.01). Does your code still work? It should,
but you may notice that your code runs more slowly, especially in cases with very large balances and interest rates. 
...
so your observations from running this experiment on the grading system might be limited to an error message complaining 
...
monthlyuppay = (balance * (1 + monthlyinterestR) ** 12) / 12.0

guessmin = (monthlylowpay + monthlyuppay)/2
remaintopay = balance
epsilon = 0.10

while remaintopay >= epsilon:
    guessmin = (monthlylowpay + monthlyuppay) / 2

	for i in range(1,13):
		newbalance = remaintopay - guessmin
		monthinterest = annualInterestRate/12*newbalance
		remaintopay = newbalance + monthinterest
	if remaintopay < 0:
		monthlyuppay = guessmin
		remaintopay = balance
	elif (remaintopay>epsilon):
		monthlylowpay = guessmin
		remaintopay = balance
print("lower payment: ",round(guessmin,2))
'''